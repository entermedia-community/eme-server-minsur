...
public class TestModule extends BaseModule
{
	...
	// TestPageLoad.testLoading
	public void runCode(WebPageRequest inContext)
	{
		if (!inContext.hasPermission("viewsettings"))
		{
			throw new RuntimeException("You do not have permission to view settings.");
		}
		String bean = inContext.getRequiredParameter("bean");
		String method = inContext.getRequiredParameter("method");
		Object object = getModuleManager().getBean(bean);
		try
		{
			object.getClass().getMethod(method, new Class[] {WebPageRequest.class}).invoke(object, new Object[] {inContext});
		}
		catch (Exception ex)
		{
